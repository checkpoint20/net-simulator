      ...
      /*  Locale locale = null;
        if(language!=null && country!=null)
        {
            ...
            if(locale==null)
            {
                ...
                System.out.println("Locale is null, using system default: "+locale);
            }else
            {
        ...
        {
            locale = Locale.getDefault();
            System.out.println("Locale is not set, using system default: "+locale);
        }
        
        try
        {
            rsc = ResourceBundle.getBundle("netsimulator", locale);
        ...
        DesktopNetworkShape.setResourceBundle(rsc);
        */

        
        setTitle( "NET-Simulator" );
...
//        getContentPane().add(l, BorderLayout.SOUTH);
        ...
        if(currentSkin==null)
        {
            ...
            logger.warning("Name of current skin is not set in config, using default '"+DEFAULTSKIN+".'");
        }
        ...
        {
            logger.severe("Skin '"+currentSkin+"' is not set in config.");
            System.exit(1);
        }
        if(skin==null && !currentSkin.equals(DEFAULTSKIN))
        {
            logger.warning("Skin '"+currentSkin+"' is not set in config. Trying default '"+DEFAULTSKIN+".'");
            currentSkin = DEFAULTSKIN;
            skin =skins.get(currentSkin);
            if(skin==null)
            {
                logger.warning("Skin '"+currentSkin+"' is not set in config.");
                System.exit(1);
            }
        }
        for(Iterator<ShapeInfo> i = skin.iterator(); i.hasNext(); )
        {   
            ShapeInfo shapeInfo = i.next();
    ...
    private void saveFile()
    {
        ...
        //{
            ...
            Thread serializingThread = new Thread(serializer);
        ...
        /*}catch(IOException io)
        {
            ...
            JOptionPane.showMessageDialog(
                this,
                "Ошибка ввода/вывода при сохранении файла.",
                "Ошибка",
                JOptionPane.ERROR_MESSAGE);
        } */    
        setTitle("NET-Simulator - "+currentFile);
    ...
    {
//        getContentPane().remove(networkPanel);
        ...
        if( !networkPanel.isSaved() )
        {
            ...
            switch(n)
            {
                 ...
                 case JOptionPane.NO_OPTION:
                     return;
                 default:
                     System.err.println("Error: not valid switch value.");
                     return;
            }
        }
